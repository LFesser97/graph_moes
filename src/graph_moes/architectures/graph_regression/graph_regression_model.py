...
from graph_moes.architectures.graph_model import ResGatedGraphConv

# from graph_moes.architectures.layers.layers import TaylorGCNConv, ComplexGCNConv
from graph_moes.architectures.layers.complex_valued_layers import UnitaryGCNConvLayer
from graph_moes.architectures.layers.real_valued_layers import OrthogonalGCNConvLayer
from graph_moes.architectures.performer import PerformerAttention
from graph_moes.utils.measure_smoothing import dirichlet_normalized
...
class GINE(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        """
        x = x.float()
        # Normalize features if requested (L2 normalization per node) - before embedding
        # Note: For GINE, normalization happens on raw input features before node embedding
        if self.normalize_features:
            x_norm = torch.norm(x, p=2, dim=1, keepdim=True)
            x = x / (x_norm + 1e-8)  # Add small epsilon to avoid division by zero
