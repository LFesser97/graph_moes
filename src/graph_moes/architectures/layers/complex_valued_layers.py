...
class ComplexDropout(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if torch.is_complex(x):
            mask = F.dropout(
                torch.ones_like(x.real), p=self.dropout, training=self.training
            )
            return x * mask
        else:
...
class TaylorGCNConv(MessagePassing):
    def __init__(
        self, conv: ComplexGCNConv, T: int = 16, return_real: bool = False, **kwargs
    ):
        super().__init__(**kwargs)
        self.conv = conv
        self.T = T
...
class UnitaryGINEConvLayer(nn.Module):
    ...
    def forward(
        self, x, edge_index, batch: OptTensor = None, edge_attr: OptTensor = None
    ):
        x_in = x
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if edge_dim is not None:
            ...
            self.edge_lin.weight = torch.nn.Parameter(
                torch.complex(
