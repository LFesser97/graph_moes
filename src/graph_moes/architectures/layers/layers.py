...
from torch_geometric.utils import is_torch_sparse_tensor, scatter, spmm, to_edge_index
...
class TaylorGCNConv(MessagePassing):
    def __init__(
        self, conv: ComplexGCNConv, T: int = 8, return_real: bool = False, **kwargs
    ):
        super().__init__(**kwargs)
        self.conv = conv
        self.T = T
    ...
    def forward(
        ...
        for k in range(self.T):
            x_k = self.conv(x_k, edge_index, edge_weight, apply_feature_lin=False) / (
                k + 1
            )
            x += x_k
        if self.return_real:
...
class UnitaryGCNConv_with_edge(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.edgelin.weight = torch.nn.Parameter(
            torch.complex(self.edgelin.weight, torch.zeros_like(self.edgelin.weight))
        )
    ...
    def forward(
        ...
        for k in range(self.T):
            x_k = (
                1j
                * self.propagate(edge_index, x=(x_k, x_k), edge_attr=edge_attr)
                / (k + 1)
            )
            out += x_k
    ...
    def forward(
        ...
        for k in range(self.T):
            x_k = (
                1j
                * self.propagate(
                    edge_index, x=(x_k, x_k), edge_attr=edge_attr, size=size
                )
                / (k + 1)
            )
            out += x_k
